...
BooleanArray::BooleanArray(const size_t count) : array{nullptr} {
  const size_t size =
      (count / BIT_SIZE_OF_BYTE) + (((count % BIT_SIZE_OF_BYTE) > 0) ? 1 : 0);
  try {
    array = new Byte[size];
  } catch (const std::bad_alloc& error) {
    throw error;
...
BooleanArray::~BooleanArray() { delete[] array; }
...
bool isWordChar(const char prev, const char current, const char next) {
  return isLetter(current) ||
         ((current == '-') && isLetter(next) && isLetter(prev));
}