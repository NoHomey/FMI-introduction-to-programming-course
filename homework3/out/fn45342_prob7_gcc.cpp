    ...
    if (isLetterSymbol && (!isWord)) {
      ...
      if (!isVowel) {
        ...
        if (isLetter(nextSymbol)) {
          ...
          symbol = (isNextLowerCase && isUpperCase) ? toUpperCase(nextSymbol)
                                                    : nextSymbol;
        }
      }
    }
...
void String::deleteString() { delete[] string; }
...
String::String() : allocated_size{0}, size{0}, string{nullptr} { clear(); }
...
char String::operator[](const size_t index) { return string[index]; }
