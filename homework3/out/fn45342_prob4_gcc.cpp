...
class PositionOnChessBoard {
 ...
 public:
  PositionOnChessBoard(const Number onBoardColumn = 0,
                       const Number onBoardRow = 0);
...
class ChessFigure {
 protected:
  ...
  virtual void generateAttackMovment(
      const PositionOnChessBoard& figurePositionOnBoard,
      ChessFigureMovment* movment) const = 0;

 public:
  ChessFigure(const char rawColumn, const Number rawRow,
              const AllianceChessFigureVector* alliance);

  ChessFigure(const PositionOnChessBoard& figurePositionOnBoard,
              const AllianceChessFigureVector* alliance);
...
class Rook : public ChessFigure {
 protected:
  void generateAttackMovment(const PositionOnChessBoard& figurePositionOnBoard,
                             ChessFigureMovment* movment) const final;

 public:
  Rook(const char rawColumn, const Number rawRow,
       const AllianceChessFigureVector* alliance);

  Rook(const PositionOnChessBoard& figurePositionOnBoard,
       const AllianceChessFigureVector* alliance);
};

class King : public ChessFigure {
 protected:
  bool static isValidPositionOnChessBoard(
      const PositionOnChessBoard& figurePositionOnBoard);

  void static addToMovmentsIfPossible(const PositionOnChessBoard& movment,
 ...
 public:
  King(const char rawColumn, const Number rawRow,
       const AllianceChessFigureVector* alliance);

  King(const PositionOnChessBoard& figurePositionOnBoard,
       const AllianceChessFigureVector* alliance);
...
class Bishop : public ChessFigure {
 ...
 public:
  Bishop(const char rawColumn, const Number rawRow,
         const AllianceChessFigureVector* alliance);

  Bishop(const PositionOnChessBoard& figurePositionOnBoard,
         const AllianceChessFigureVector* alliance);
};
...
class Queen : public ChessFigure {
 ...
 public:
  Queen(const char rawColumn, const Number rawRow,
        const AllianceChessFigureVector* alliance);

  Queen(const PositionOnChessBoard& figurePositionOnBoard,
        const AllianceChessFigureVector* alliance);
...
class Knight : public ChessFigure {
 ...
 public:
  Knight(const char rawColumn, const Number rawRow,
         const AllianceChessFigureVector* alliance);

  Knight(const PositionOnChessBoard& figurePositionOnBoard,
         const AllianceChessFigureVector* alliance);
};
...
class KingVsAllOutcomer {
 ...
 public:
  ...
  void setFigure(const ChessFigureType figureType, const char rawColumn,
                 const Number rawRow);
...
int main() {
  ...
  for (Number index = 0; index < count; ++index) {
    ...
    outcomer.setFigure(KingVsAllOutcomer::charToChessFigureType(figureType),
...
PositionOnChessBoard::PositionOnChessBoard(const Number onBoardColumn,
                                           const Number onBoardRow)
    : column{onBoardColumn}, row{onBoardRow} {}

Number PositionOnChessBoard::getRow() const { return row; }

Number PositionOnChessBoard::getColumn() const { return column; }

bool PositionOnChessBoard::operator==(
    const PositionOnChessBoard& otherPositionOnChessBoard) const {
  return (row == otherPositionOnChessBoard.row) &&
         (column == otherPositionOnChessBoard.column);
}

template <typename T>
SpecializedVector<T>::SpecializedVector() : count{0} {}
...
ChessFigureMovment::ChessFigureMovment() : SpecializedVector{}, positions{} {}
...
AllianceChessFigureVector::~AllianceChessFigureVector() { delete[] figures; }

void AllianceChessFigureVector::push(const ChessFigure* element) {
  figures[count++] = element;
}

void AllianceChessFigureVector::removeTemporary(
    const PositionOnChessBoard& positionOnBoard) {
  Number index = 0;
...
void AllianceChessFigureVector::restore() { count++; }

const ChessFigure* AllianceChessFigureVector::operator[](
    const Number index) const {
  return figures[index];
}

bool AllianceChessFigureVector::isAllyAtPosition(
    const PositionOnChessBoard& positionOnBoard) const {
  for (Number index = 0; index < count; ++index) {
    if (figures[index]->getPositionOnBoard() == positionOnBoard) {
      return true;
    }
...
ChessFigure::ChessFigure(const char rawColumn, const Number rawRow,
                         const AllianceChessFigureVector* alliance)
    : positionOnBoard{static_cast<Number>(rawColumn - 'a'),
                      static_cast<Number>(rawRow - 1)},
      allies{alliance} {}

ChessFigure::ChessFigure(const PositionOnChessBoard& figurePositionOnBoard,
                         const AllianceChessFigureVector* alliance)
...
Rook::Rook(const char rawColumn, const Number rawRow,
           const AllianceChessFigureVector* alliance)
    : ChessFigure{rawColumn, rawRow, alliance} {}

Rook::Rook(const PositionOnChessBoard& figurePositionOnBoard,
           const AllianceChessFigureVector* alliance)
    : ChessFigure{figurePositionOnBoard, alliance} {}

void Rook::generateAttackMovment(
    const PositionOnChessBoard& figurePositionOnBoard,
    ChessFigureMovment* movment) const {
  ...
  if (row == figureRow) {
    ...
    for (Number movmentColumn = column + movmentStep;
         movmentColumn != figureColumn; movmentColumn += movmentStep) {
    ...
    for (Number movmentRow = row + movmentStep; movmentRow != figureRow;
         movmentRow += movmentStep) {
...
bool King::isValidPositionOnChessBoard(
    const PositionOnChessBoard& figurePositionOnBoard) {
  return (figurePositionOnBoard.getRow() <= 7) &&
         (figurePositionOnBoard.getColumn() <= 7);
}

void King::addToMovmentsIfPossible(const PositionOnChessBoard& movment,
                                   ChessFigureMovment& possibleMovments) {
...
King::King(const char rawColumn, const Number rawRow,
           const AllianceChessFigureVector* alliance)
    : ChessFigure{rawColumn, rawRow, alliance} {}

King::King(const PositionOnChessBoard& figurePositionOnBoard,
           const AllianceChessFigureVector* alliance)
    : ChessFigure{figurePositionOnBoard, alliance} {}
...
void King::generateAttackMovment(
    const PositionOnChessBoard& figurePositionOnBoard,
    ChessFigureMovment* movment) const {
...
Bishop::Bishop(const char rawColumn, const Number rawRow,
               const AllianceChessFigureVector* alliance)
    : ChessFigure{rawColumn, rawRow, alliance} {}

Bishop::Bishop(const PositionOnChessBoard& figurePositionOnBoard,
               const AllianceChessFigureVector* alliance)
    : ChessFigure{figurePositionOnBoard, alliance} {}

void Bishop::generateAttackMovment(
    const PositionOnChessBoard& figurePositionOnBoard,
    ChessFigureMovment* movment) const {
  const Number row = positionOnBoard.getRow();
  const Number figureRow = figurePositionOnBoard.getRow();
  ...
  if ((rowStep * rowDiff) == (columnStep * columnDiff)) {
    ...
    for (; movmentRow != figureRow;
         movmentRow += rowStep, movmentColumn += columnStep) {
...
Queen::Queen(const char rawColumn, const Number rawRow,
             const AllianceChessFigureVector* alliance)
    : ChessFigure{rawColumn, rawRow, alliance} {}

Queen::Queen(const PositionOnChessBoard& figurePositionOnBoard,
             const AllianceChessFigureVector* alliance)
    : ChessFigure{figurePositionOnBoard, alliance} {}

void Queen::generateAttackMovment(
    const PositionOnChessBoard& figurePositionOnBoard,
    ChessFigureMovment* movment) const {}
...
Knight::Knight(const char rawColumn, const Number rawRow,
               const AllianceChessFigureVector* alliance)
    : ChessFigure{rawColumn, rawRow, alliance} {}

Knight::Knight(const PositionOnChessBoard& figurePositionOnBoard,
               const AllianceChessFigureVector* alliance)
    : ChessFigure{figurePositionOnBoard, alliance} {}

void Knight::generateAttackMovment(
    const PositionOnChessBoard& figurePositionOnBoard,
    ChessFigureMovment* movment) const {
  const Number row = positionOnBoard.getRow();
  const Number column = positionOnBoard.getColumn();
  ...
  if ((PositionOnChessBoard{columnPlusOne, rowMinusTwo} ==
       figurePositionOnBoard) ||
      (PositionOnChessBoard{columnMinusOne, rowMinusTwo} ==
       figurePositionOnBoard)) {
    movment->push({column, rowMinusOne});
    movment->push({column, rowMinusTwo});
    threatening = true;
  }
  if ((PositionOnChessBoard{columnPlusOne, rowPlusTwo} ==
       figurePositionOnBoard) ||
      (PositionOnChessBoard{columnMinusOne, rowPlusTwo} ==
       figurePositionOnBoard)) {
    movment->push({column, rowPlusOne});
    movment->push({column, rowPlusTwo});
    threatening = true;
  }
  if ((PositionOnChessBoard{columnMinusTwo, rowPlusOne} ==
       figurePositionOnBoard) ||
      (PositionOnChessBoard{columnMinusTwo, rowMinusOne} ==
       figurePositionOnBoard)) {
    movment->push({columnMinusOne, row});
    movment->push({columnMinusTwo, row});
    threatening = true;
  }
  if ((PositionOnChessBoard{columnPlusTwo, rowPlusOne} ==
       figurePositionOnBoard) ||
      (PositionOnChessBoard{columnPlusTwo, rowMinusOne} ==
       figurePositionOnBoard)) {
...
bool KingVsAllOutcomer::kingHasNoMove() {
  const ChessFigureMovment possibleMovmentsOfOpposingKing =
      opposingKing.allPossibleMovments();
...
KingVsAllOutcomer::ChessFigureType KingVsAllOutcomer::charToChessFigureType(
    const char symbol) {
  switch (symbol) {
    case 'Q':
      return queen;
...
void KingVsAllOutcomer::setFigure(const ChessFigureType figureType,
                                  const char rawColumn, const Number rawRow) {
  switch (figureType) {
