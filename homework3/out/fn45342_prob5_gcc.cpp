...
class Number {
 private:
  ...
  static CharToNumberConverter getCharToDigitConverter(
      const Digit baseOfNumericSystem);
...
Array::Array() : capacity{0}, size{0}, data{nullptr} {
  try {
...
size_t Array::length() const { return size; }

Digit Array::operator[](const size_t index) const { return data[index]; }
...
Digit Number::CharToHexNumberConverter(const char symbol) {
  return symbol <= NINE_CHAR ? CharToDecimalLikeNumberConverter(symbol)
                             : (TEN_DIGIT + (symbol - A_CHAR));
}

CharToNumberConverter Number::getCharToDigitConverter(
    const Digit baseOfNumericSystem) {
...
bool Number::isEndReached() const { return reachedEnd; }