...
#define CHECK_FIELD_MATCH(checked, requirements, field) (checked->field >= requirements->field)
...
MobilePlan(const Price price_ = 0, const Number minutes_ = 0, const Number megaBytes_ = 0, const Number smses_ = 0);
} typedef MobilePlan;

template<typename T>
T input();

bool plansMatch(const MobilePlan* const checked, const MobilePlan* const requirements);
...
MobilePlan::MobilePlan(const Price price_, const Number minutes_, const Number megaBytes_, const Number smses_)
