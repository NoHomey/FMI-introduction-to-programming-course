...
char getLetterOfIntersection(const PositionInEncodingTable& rowInTable, const PositionInEncodingTable& columnInTable) const;
...
char toUpperCase(const char letter) {
    return isLowerCaseLetter(letter) ? (letter - DISTANCE_FROM_UPPER_TO_LOWER_CASE) : letter;
}

void EncodableString::deleteString() {
    delete[] string;
}
...
size_t EncodableString::length() const {
    return size;
}
...
char EncodableString::get(const size_t index) const {
    return string[index];
}

const char* const EncodableString::operator*() {
    return string;
}

EncodingTable::LetterPair::LetterPair(const char firstLetter, const char secondLetter)
: first{firstLetter}, second{secondLetter} {}

char EncodingTable::LetterPair::getFirst() const {
    return first;
}

char EncodingTable::LetterPair::getSecond() const {
    return second;
}
...
EncodingTable::PositionInEncodingTable::PositionInEncodingTable(const Index r, const Index c)
: row{r}, column{c} {}

Index EncodingTable::PositionInEncodingTable::getRow() const {
    return row;
}
...
bool EncodingTable::PositionInEncodingTable::isInSameRow(const PositionInEncodingTable& otherPosition) const {
    return row == otherPosition.row;
}

bool EncodingTable::PositionInEncodingTable::isInSameColumn(const PositionInEncodingTable& otherPosition) const {
...
char EncodingTable::replace(const char symbol) {
    ...
    return (isUpperCaseLetter(upperCased) && (upperCased != notInTable)) ? upperCased : replacer;
}
...
bool EncodingTable::isReplacerPair(const LetterPair& letterPair) const {
    return isReplacer(letterPair.getFirst()) && isReplacer(letterPair.getSecond());
}

EncodingTable::PositionInEncodingTable EncodingTable::positionInEncodingTable(const char letter) const {
...
char EncodingTable::nextLetterInRow(const PositionInEncodingTable& positionInTable) const {
    const Index column = positionInTable.getColumn();
    const Index columnOfNextLetterInRow = column == (DIMENTION - 1) ? FIRST_INDEX : (column + 1);

    return table[positionInTable.getRow()][columnOfNextLetterInRow];
}

char EncodingTable::nextLetterInColumn(const PositionInEncodingTable& positionInTable) const {
    const Index row = positionInTable.getRow();
    const Index rowOfNextLetterInColumn = row == (DIMENTION - 1) ? FIRST_INDEX : (row +  1);
    
    return table[rowOfNextLetterInColumn][positionInTable.getColumn()];
}

char EncodingTable::getLetterOfIntersection(const PositionInEncodingTable& rowInTable, const PositionInEncodingTable& columnInTable) const {
    return table[rowInTable.getRow()][columnInTable.getColumn()];
}

EncodingTable::LetterPair EncodingTable::encodeLetterPair(const LetterPair&& letterPair) const {
    ...
    const PositionInEncodingTable positionOfFirst = positionInEncodingTable(letterPair.getFirst());
    const PositionInEncodingTable positionOfSecond = positionInEncodingTable(letterPair.getSecond());
    if(positionOfFirst.isInSameRow(positionOfSecond)) {
        return {nextLetterInRow(positionOfFirst), nextLetterInRow(positionOfSecond)};
    }
    if(positionOfFirst.isInSameColumn(positionOfSecond)) {
        return {nextLetterInColumn(positionOfFirst), nextLetterInColumn(positionOfSecond)};
    }

    return {
        getLetterOfIntersection(positionOfFirst, positionOfSecond),
...
EncodingTable::EncodingTable()
: row{FIRST_INDEX}, column{FIRST_INDEX}, replacer{'Z'}, notInTable{'\0'}, table{} {}
