...
template<typename K, typename V>
Map<K, V>::Map()
: size{0}, entries{nullptr} {}

template<typename K, typename V>
Map<K, V>::Map(const MapEntry& entry)
: size{0}, entries{nullptr} {
    try {
...
Outputer::Outputer(const unsigned int& fn)
: student{fn} {}

void Outputer::operator()(const unsigned short& task, const float& score) const {
