...
class NumberDigitStriker : public Generator<Number, NumberRaisedOnPower> {
 protected:
  ...
  Number yield(
      const NumberRaisedOnPower curretValueOfBase10RaisedOnPower) const;
...
NumberRaisedOnPower::operator Number() const { return numberRaisedOnPower; }
...
Number NumberDigitStriker::yield(
    const NumberRaisedOnPower curretValueOfBase10RaisedOnPower) const {
  return ((numberWithEachDigitBeeingStriked /
           (curretValueOfBase10RaisedOnPower + 1)) *
          curretValueOfBase10RaisedOnPower) +
         (numberWithEachDigitBeeingStriked % curretValueOfBase10RaisedOnPower);
}

NumberDigitStriker::NoDigitToStrikeFrom::NoDigitToStrikeFrom(
    const Number numberWithStrikedDigits)
    : std::out_of_range("No more digits to strike from " +
