...
public:
MultiplierSequnce(const PrimeNumberGenerator& primes, const Number end, const Number start = 2);
...
int main() {
    ...
    if(n > 1) {
        ...
        for(Number k = 3; k < n; ++k) {
            printWithSpace(
                MultiplierSequnce(primes, n, k).reduce(
                    MultiplierSequnce(primes, n - k + 1)
                )
            );
        }
        if(n > 2) {
...
bool EveryNumber::every(const Number test, Array::const_iterator iterator) const {
...
EveryNumber::EveryNumber(const Array& vector, const EveryNumberPredicate predicate)
...
bool PrimeNumberGenerator::isNotDivisible(const Number checked, const Number prime) {
...
MultiplierSequnce::Multiplier::Multiplier()
: result{1} {}

void MultiplierSequnce::Multiplier::operator()(const Number n) {
    result *= n;
}

Number MultiplierSequnce::Multiplier::product() const {
    return result;
}

MultiplierSequnce::MultiplierSequnce(const PrimeNumberGenerator& primes, const Number end, const Number start)
: multipliers{} {
...
Array::iterator MultiplierSequnce::begin() {
    return multipliers.begin();
}

Array::iterator MultiplierSequnce::end() {
    return multipliers.end();
}
...
Number MultiplierSequnce::reduce(MultiplierSequnce&& divisor) {
    ...
    Array::iterator differenceEnd = std::set_difference(
        beginIterator,
        endIterator,
        intersection.begin(),
        intersection.end(),
