...
int main() {
    ...
    for(Number i = 0; i < nq; ++i) {
        querry = {input};
        std::cout << max(
            P_AdicOrder(querry.p1, querry.n),
...
Querry::Querry(Number (*source)())
: p1{source()}, n{source()}, p2{source()} {}
...
P_AdicOrder::P_AdicOrder(Number p, Number n)
: infinity{n == 0}, value{0} {
    if(!isInfinity()) {
        Number result = 1;
...
bool P_AdicOrder::isInfinity() const {
    return infinity;
}
