...
virtual Point calculatePointOfIntersectionWithLine(const Line& line) const = 0;
};
...
public:
class PointIsInsideRectangleWithCentralSymmetry: public std::invalid_argument {
private:
static std::string pointToString(const Point& point);

public:
PointIsInsideRectangleWithCentralSymmetry(const Point& pointinSidePlaygroundPoint, const Point& pointOfCentralSymmetry);
};
...
int main() {
    ...
    for(size_t pointsCounter = 1; pointsCounter < n; ++pointsCounter) {
        ...
        if(isAInPlayground || isBInPlayground) {
            ...
            playerTotalDistance += f(Vector(playerPosition1, playerPosition2).distance());
        }
    }
...
Number Point::getX() const {
    return x;
}

Number Point::getY() const {
    return y;
}
...
Number Vector::calculateXDiff() const {
    return std::abs(a1);
}

Number Vector::calculateYDiff() const {
    return std::abs(a2);
}

Number Vector::distance() const {
    return std::sqrt((a1 * a1) + (a2 * a2));
}

bool Side::isType(const SideType sideType) const {
    return type == sideType;
}

Side::Side(const Number common, const SideType sideType)
: forAll{common}, type{sideType} {}

Number Side::getCommon() const {
    return forAll;
}

SideType Side::getType() const {
    return type;
}

bool Side::isXType() const {
    return isType(SideType::X);
}

bool Side::isYType() const {
    return isType(SideType::Y);
}

bool Side::isUnknownType() const {
    return isType(SideType::Unknown);
}

XSide::XSide(const Number common)
: Side(common, SideType::X) {}

Number XSide::getX() const {
    return getCommon();
}

YSide::YSide(const Number common)
: Side(common, SideType::Y) {}

Number YSide::getY() const {
    return getCommon();
}

Line::SideIsOfUnknownType::SideIsOfUnknownType()
: std::invalid_argument("Can not calculatePointFromIntersectionWithSide which type is SideType::Unknown\n") {}
...
Number Line::calculateX(const Number y) const {
    return (c + (b * y)) / (-a);
}

Number Line::calculateY(const Number x) const {
    return (c + (a * x)) / (-b);
}
...
: A{downLeft}, C{upRight}, AB{A.getY()}, BC{C.getX()}, DC{C.getY()}, AD{A.getX()} {}

Point RectangleWithCentralSymmetry::pointWithPositiveCoordinates(const Point& point) {
    return {std::abs(point.getX()), std::abs(point.getY())};
}

Point RectangleWithCentralSymmetry::pointWithNegativeCoordinates(const Point& point) {
...
bool RectangleWithCentralSymmetry::isInRectanglesSide(const Number d, const Side& dSide) {
    return !(isGreater(std::abs(d), dSide.getCommon()));
}

std::string RectangleWithCentralSymmetry::PointIsInsideRectangleWithCentralSymmetry::pointToString(const Point& point) {
    return "Point with coordinates ("
        + std::to_string(point.getX()) + ", "
        + std::to_string(point.getY()) + ")";
}

RectangleWithCentralSymmetry::PointIsInsideRectangleWithCentralSymmetry::PointIsInsideRectangleWithCentralSymmetry(
...
: RectangularShape(pointWithNegativeCoordinates(point), pointWithPositiveCoordinates(point)) {}
...
Point RectangleWithCentralSymmetry::calculatePointOfIntersectionWithLine(const Line& line) const {
