...
int main() {  
    ...
    if(lengthOfDigitsOfA <= lengthOfDigitsOfB) {
        ...
        for(Number indexOfDigitsOfB = BEGINING; indexOfDigitsOfB <= lastComparsionBegining; ++indexOfDigitsOfB) { 
            for(indexOfDigitsOfA = BEGINING; indexOfDigitsOfA < lengthOfDigitsOfA; ++indexOfDigitsOfA) { 
                if(digitsOfB[indexOfDigitsOfB + indexOfDigitsOfA] != digitsOfA[indexOfDigitsOfA]) {
                    break;
                }
            }
...
Number Digits::operator[](const Number index) const { 
    return digits[index]; 
}

Number Digits::length() const { 
    return size; 
}